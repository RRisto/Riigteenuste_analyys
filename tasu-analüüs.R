...
#kas tasutliste vs taslisute teenuste rahulolu on erinev
makseRahulolu=subset(andmedPikk, naitaja=="rahulolu")
library(ggplot2)
ggplot(subset(andmedPikk, naitaja=="rahulolu"), aes(x=makse, y=value))+
  geom_boxplot()

#osutamiste arv maksete lõikes
ggplot(subset(andmedPikk, naitaja=="osutamistearv"), aes(x=makse, y=value))+
  geom_boxplot()

#teenuse kulu
ggplot(subset(andmedPikk, naitaja=="halduskulu"), aes(x=makse, y=value))+
  geom_boxplot()

#ajakulu
ggplot(subset(andmedPikk, naitaja=="ajakulu"), aes(x=makse, y=value))+
  geom_boxplot()

#kliendi kogu ajakulu
ajakulu=subset(andmedPikk, naitaja=="ajakulu")
osutamistearv=subset(andmedPikk, naitaja=="osutamistearv")
ajakulu$kogu_ajakulu=osutamistearv$value*ajakulu$value

ggplot(ajakulu, aes(x=makse, y=kogu_ajakulu))+
  geom_boxplot()

####mis kanalid on tasuliste ja tasuta teenuste osutamisel
kanalidMakse=andmedPikk[!duplicated(andmedPikk$identifikaator),]

ggplot(kanalidMakse, aes(x=makse, y=kanal))+
